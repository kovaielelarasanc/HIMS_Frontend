...
import { createPrescription, getPharmacyActiveContext, listPrescriptions  } from '@/api/pharmacy'
...
const newRow = () => ({
    ...
    night: false,
    duration_days: 3,
    // quantity is auto-computed on save; we show computed preview in UI
    route: 'po',
    instructions: '',
})
...
export default function CreatePrescription() {
    ...
    const save = async () => {
        ...
        const payload = {
            ...
            // Let server auto-resolve; you may also send `context` to be explicit:
            // context: context && context.type ? context : undefined,
            notes,
            items: rows.map(r => ({
                ...
                quantity: computeQty(r),       // we send it; server will also check
                route: r.route || 'po',
