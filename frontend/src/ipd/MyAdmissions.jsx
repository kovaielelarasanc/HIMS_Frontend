    ...
    useEffect(() => {
        ...
        const run = async () => {
            ...
            const [a, b] = await Promise.all([
                listAdmissions({ status: 'admitted', practitioner_user_id: me.id }),
                listBeds()
            ])
            if (!alive) return
            setRows(a.data || [])
            ...
            <div className="rounded-xl border bg-white overflow-hidden">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        {!rows?.length && <tr><td className="p-4 text-sm text-gray-500" colSpan={5}>No admissions assigned to you</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
