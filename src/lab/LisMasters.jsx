...
export default function LisMasters() {
  ...
  // Ensure every draft row always has a unique key (prevents “remove one removes all”)
  useEffect(() => {
    setDraftRows((prev) => {
      let changed = false
  ...
  const stopKeys = useCallback((e) => {
    // blocks app-level shortcuts / tab switching while typing
    e.stopPropagation()
  }, [])
  ...
  const addDraftRow = useCallback(() => {
    ...
    setDraftRows((prev) => [
      ...prev,
      { ...EMPTY_DRAFT_SERVICE, __key: mkKey() },
    ])
  }, [selectedDept])
  ...
  const toggleDraftAdv = useCallback((key) => {
    setDraftRows((prev) =>
      prev.map((r) =>
        r.__key === key ? { ...r, __advanced: !r.__advanced } : r
      )
    )
  }, [])
  ...
  async function saveServiceEdit() {
    ...
    const payload = {
      ...
      display_order:
        serviceEditBuffer.display_order !== '' ? Number(serviceEditBuffer.display_order) : null,
    }
      ...
      <div key={dept.id} className="mb-1">
        <div
          className={`group flex items-center justify-between rounded-2xl px-3 py-2 text-sm cursor-pointer border transition-all ${
            isSelected ? 'border-transparent' : 'border-transparent hover:border-slate-200'
          }`}
          style={{
      ...
      <div className="grid flex-1 gap-4 xl:grid-cols-[340px,1fr] lg:grid-cols-[300px,1fr]">
        ...
        <div className="flex flex-col rounded-3xl border border-slate-200/70 bg-white p-3 shadow-sm">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div>
              <h2 className="text-sm font-semibold text-slate-800">
                {selectedDept ? `Services for ${selectedDept.name}` : 'Select a department'}
              </h2>
              <p className="text-[11px] text-slate-500">
                Main fields stay clean. Advanced fields are optional via “Details”.
              </p>
            </div>
          ...
          {selectedDept && canCreateServices && (
            ...
            >
              <div className="flex flex-col gap-2 border-b border-slate-200/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="text-sm font-semibold text-slate-900">Bulk Add</div>
                  <div className="text-xs text-slate-600">
                    Main: Name / Code / Unit / Range / Comments. Advanced only when needed.
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    • Dept: <span className="font-semibold">{selectedDept.name}</span>
                  </span>
                  ...
                  <div className="grid gap-3">
                    ...
                    {draftRows.map((r) => (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 gap-3 xl:grid-cols-12 xl:gap-3">
                          <div className="xl:col-span-4">
                            ...
                            <input
                              ...
                              value={r.name}
                              onChange={(e) => updateDraft(r.__key, 'name', e.target.value)}
                            />
                          </div>
                          ...
                          <div className="xl:col-span-2">
                            ...
                            <input
                              ...
                              value={r.code}
                              onChange={(e) => updateDraft(r.__key, 'code', e.target.value)}
                            />
                          </div>
                          ...
                          <div className="xl:col-span-2">
                            ...
                            <input
                              ...
                              placeholder="-"
                              value={r.unit}
                              onChange={(e) => updateDraft(r.__key, 'unit', e.target.value)}
                            />
                          </div>
                          ...
                          <div className="xl:col-span-2">
                            ...
                            <input
                              ...
                              onChange={(e) => updateDraft(r.__key, 'normal_range', e.target.value)}
                            />
                          </div>
                        ...
                        {/* Comments (always visible as MAIN field) */}
                        <div className="mt-3">
                          <label className="text-[11px] font-semibold text-slate-600">
                            Comments
                          </label>
                          <input
                            className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none
                              focus:border-slate-300 focus:ring-4 focus:ring-slate-200/60"
                            placeholder="Optional default comments for report…"
                            value={r.comments_template}
                            onChange={(e) =>
                              updateDraft(r.__key, 'comments_template', e.target.value)
                            }
                          />
                        </div>

                        {/* Advanced (only on click) */}
                        {r.__advanced && (
                            ...
                            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
                              <div>
                                ...
                                <input
                                  ...
                                  value={r.sample_type}
                                  onChange={(e) =>
                                    updateDraft(r.__key, 'sample_type', e.target.value)
                                  }
                                />
                              </div>
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  value={r.method}
                                  onChange={(e) =>
                                    updateDraft(r.__key, 'method', e.target.value)
                                  }
                                />
                              </div>
              ...
              <table className="min-w-full border-separate border-spacing-0 text-xs">
                <thead className="sticky top-0 z-10 bg-slate-50">
                  <tr>
                    ...
                    <th className="border-b border-slate-200 px-3 py-2 text-left font-medium text-slate-600">Service Name</th>
                    <th className="border-b border-slate-200 px-3 py-2 text-left font-medium text-slate-600">Code</th>
                    <th className="border-b border-slate-200 px-3 py-2 text-left font-medium text-slate-600">Unit</th>
                    <th className="border-b border-slate-200 px-3 py-2 text-left font-medium text-slate-600">Normal Range</th>
                    <th className="border-b border-slate-200 px-3 py-2 text-left font-medium text-slate-600">Sample Type</th>
                    <th className="border-b border-slate-200 px-3 py-2 text-left font-medium text-slate-600">Method</th>
                    <th className="border-b border-slate-200 px-3 py-2 text-left font-medium text-slate-600">Comments</th>
                    <th className="border-b border-slate-200 px-3 py-2 text-left font-medium text-slate-600">Active</th>
                    <th className="border-b border-slate-200 px-3 py-2 text-right font-medium text-slate-600">Actions</th>
                  </tr>
                </thead>
                ...
                <tbody>
                  {filteredServices.map((svc, idx) => {
                    ...
                    return (
                      <tr key={svc.id} className={idx % 2 ? 'bg-slate-50/40' : 'bg-white'}>
                        <td className="border-b border-slate-100 px-3 py-1.5 text-[11px] text-slate-500">{idx + 1}</td>
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5">
                          <input
                            ...
                            value={buf?.name || ''}
                            onChange={(e) => handleServiceEditChange('name', e.target.value)}
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5">
                          <input
                            ...
                            className="w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-[11px] text-slate-800 focus:border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-200 disabled:bg-slate-50"
                            value={buf?.code || ''}
                            onChange={(e) => handleServiceEditChange('code', e.target.value)}
                            disabled={!canUpdateServices || !isEditing}
                          />
                        </td>
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5">
                          <input
                            ...
                            placeholder="-"
                            value={buf?.unit || ''}
                            onChange={(e) => handleServiceEditChange('unit', e.target.value)}
                            disabled={!canUpdateServices || !isEditing}
                          />
                        </td>
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5">
                          <input
                            ...
                            onChange={(e) => handleServiceEditChange('normal_range', e.target.value)}
                            disabled={!canUpdateServices || !isEditing}
                          />
                        </td>
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5">
                          <input
                            ...
                            onChange={(e) => handleServiceEditChange('sample_type', e.target.value)}
                            disabled={!canUpdateServices || !isEditing}
                          />
                        </td>
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5">
                          <input
                            ...
                            className="w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-[11px] text-slate-800 focus:border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-200 disabled:bg-slate-50"
                            value={buf?.method || ''}
                            onChange={(e) => handleServiceEditChange('method', e.target.value)}
                            disabled={!canUpdateServices || !isEditing}
                          />
                        </td>
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5">
                          <textarea
                            ...
                            onChange={(e) => handleServiceEditChange('comments_template', e.target.value)}
                            disabled={!canUpdateServices || !isEditing}
                          />
                        </td>
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5">
                          <input
                            ...
                            onChange={(e) => handleServiceEditChange('is_active', e.target.checked)}
                            disabled={!canUpdateServices || !isEditing}
                          />
                        </td>
          ...
          <div className="w-full max-w-md rounded-3xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  {deptForm.parent_id ? 'This will be created as a sub-department.' : 'Top level department.'}
                </p>
              </div>
              <button
                type="button"
