...
export default function BillingPrintDownload({ caseId, caseNumber, patientName, uhid }) {
    const TEMPLATES = useMemo(
        () => [
            ...
            {
                key: "FULL_CASE",
                label: "Full Case (Recommended)",
                desc: "Invoices + Payments + Advances + Insurance + Final Summary",
                legacyKind: "FULL_CASE",
                filename: () => `Billing-FullCase-${caseNumber || `CASE-${caseId}`}.pdf`,
            },
        ],
        [caseId, caseNumber]
    )
