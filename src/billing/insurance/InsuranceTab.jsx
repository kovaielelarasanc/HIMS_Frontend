...
import {
    insGet, insUpsert,
    insLines, insPatchLines, insSplit,
    preauthList, preauthCreate, preauthSubmit, preauthApprove, preauthPartial, preauthReject,
    claimList, claimCreate, claimSubmit, claimSettle, claimDeny, claimQuery
...
function suggestFromCase(caseInfo) {
    ...
    if (caseInfo.default_tpa_id) {
        return {
            ...
            insurance_company_id: (ct === "PAYER" || ct === "INSURER" || ct === "INSURANCE") && caseInfo.default_payer_id
...
export default function InsuranceTab({ caseId, canManage = true }) {
    ...
    const approvalNeeded = useMemo(() => {
        return lines.some(r => !!r.requires_preauth && toNum(r.insurer_pay_amount) > 0 && String(r.is_covered || "NO").toUpperCase() !== "NO")
    }, [lines])

    const approvalSuggestedAmount = useMemo(() => {
        return lines.reduce((sum, r) => {
            const ok = !!r.requires_preauth && toNum(r.insurer_pay_amount) > 0 && String(r.is_covered || "NO").toUpperCase() !== "NO"
            return ok ? sum + toNum(r.insurer_pay_amount) : sum
        }, 0)
    }, [lines])

    const latestApproval = useMemo(() => (preauths?.[0] || null), [preauths]) // list is desc
    ...
    const loadAll = async () => {
        ...
        try {
            const [insRes, lineRes, metaRes, caseRes] = await Promise.all([
                insGet(caseId),
                insLines(caseId),
                API.get("/billing/meta/payers"),
                API.get(`/billing/cases/${caseId}`),
            ])
            ...
            if (insRes) {
                const [pa, cl] = await Promise.all([preauthList(caseId), claimList(caseId)])
                setPreauths(pa || [])
                setClaims(cl || [])
            } else {
                setPreauths([])
        ...
        try {
            ...
            setLines((prev) =>
                prev.map((r) => {
                    ...
                    const insurer = (payload.insurer_pay_amount != null)
    ...
    const doSplit = async () => {
        try {
            ...
            if (String(msg).includes("has payments")) {
                ...
                // optional: open a confirm dialog:
                // setConfirmForceSplitOpen(true)
            } else {
                toast.error(msg)
            }
        }
    ...
    const handleSubmit = (caseId, latestClaim) => {
        console.log(caseId, "++++++++++++++++++++++++", latestClaim);

        try {
            claimSubmit(caseId, latestClaim).then(loadAll).catch((e) => toast.error(e?.response?.data?.detail || "Submit failed"))
        } catch (error) {
                                    ...
                                    <div className="rounded-lg border p-3 flex items-start justify-between gap-3">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            <Button
                                                size="sm"
                                                variant="outline"
                                                onClick={() => preauthSubmit(caseId, latestApproval.id).then(loadAll).catch((e) => toast.error(e?.response?.data?.detail || "Send failed"))}
                                            ...
                                            <Button
                                                size="sm"
                                                onClick={() => preauthApprove(caseId, latestApproval.id, { approved_amount: toNum(latestApproval.requested_amount), remarks: "" }).then(loadAll).catch((e) => toast.error(e?.response?.data?.detail || "Approve failed"))}
                                            ...
                                            <Button
                                                size="sm"
                                                variant="secondary"
                                                onClick={() => preauthPartial(caseId, latestApproval.id, { approved_amount: Math.max(0, toNum(latestApproval.requested_amount) * 0.5), remarks: "" }).then(loadAll).catch((e) => toast.error(e?.response?.data?.detail || "Partial failed"))}
                                                disabled={!canManage || String(latestApproval.status).toUpperCase() !== "SUBMITTED"}
                                            >
                                                Partial
                                            </Button>
                                            ...
                                            <Button
                                                size="sm"
                                                variant="destructive"
                                                onClick={() => preauthReject(caseId, latestApproval.id, { approved_amount: 0, remarks: "" }).then(loadAll).catch((e) => toast.error(e?.response?.data?.detail || "Reject failed"))}
                                                disabled={!canManage || String(latestApproval.status).toUpperCase() !== "SUBMITTED"}
                                            >
                                                Reject
                                            </Button>
                            ...
                            <div className="rounded-lg border p-3 flex items-start justify-between gap-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        onClick={() => submitExistingClaim(caseId, latestClaim.id)}
                                    ...
                                    <Button
                                        size="sm"
                                        variant="secondary"
                                        onClick={() => claimQuery(caseId, latestClaim.id, {
                                            approved_amount: toNum(latestClaim.approved_amount),
                                            settled_amount: toNum(latestClaim.settled_amount),
                                            remarks: ""
                                        }).then(loadAll).catch((e) => toast.error(e?.response?.data?.detail || "Query failed"))}
                                        disabled={!canManage}
                                    >
                                        Under Query
                                    </Button>
                                    ...
                                    <Button
                                        size="sm"
                                        onClick={() => claimSettle(caseId, latestClaim.id, {
                                            approved_amount: toNum(latestClaim.claim_amount),
                                            settled_amount: toNum(latestClaim.claim_amount),
                                            remarks: ""
                                        }).then(loadAll).catch((e) => toast.error(e?.response?.data?.detail || "Settle failed"))}
                                        disabled={!canManage}
                                    >
                                        Mark Settled
                                    </Button>
                                    ...
                                    <Button
                                        size="sm"
                                        variant="destructive"
                                        onClick={() => claimDeny(caseId, latestClaim.id, { approved_amount: 0, settled_amount: 0, remarks: "" }).then(loadAll).catch((e) => toast.error(e?.response?.data?.detail || "Deny failed"))}
                                        disabled={!canManage}
                                    >
                                        Deny
                                    </Button>
