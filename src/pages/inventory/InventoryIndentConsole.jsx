// FILE: src/pages/inventory/InventoryIndentConsole.jsx
...
export default function InventoryIndentConsole() {
    ...
    const [tab, setTab] = useState(canIndentView ? "indents" : "issues")
    ...
    const bumpRefetch = () => (refetchKey.current += 1)
    ...
    const loadCatalog = async () => {
        try {
            ...
            const [locs, its] = await Promise.all([
                ...
                invListItems({ is_active: true, item_type: null }),
            ])
            setLocations(locs || [])
            setItems(its || [])
        } catch (e) {
    ...
    const loadLists = async () => {
        if (tab === "indents" && !canIndentView) return
        if (tab === "issues" && !canIssueView) return
        ...
        try {
            ...
            if (tab === "indents") {
                const rows = await invListIndents(baseParams)
                const s = q.trim().toLowerCase()
                const final = !s
                    ? rows
                    : (rows || []).filter((r) =>
                        String(r.indent_number || "").toLowerCase().includes(s)
                    )
                setIndentRows(final || [])
            } else {
                const rows = await invListIssues(baseParams)
                const s = q.trim().toLowerCase()
                const final = !s
                    ? rows
                    : (rows || []).filter((r) =>
                        String(r.issue_number || "").toLowerCase().includes(s)
                    )
                setIssueRows(final || [])
            }
        } catch (e) {
    ...
    const addLine = (item) => {
        ...
        setCreatePayload((p) => ({
            ...
            items: [
                ...p.items,
                { item_id: item.id, requested_qty: 1, is_stat: false, remarks: "" },
            ],
        }))
    }
    ...
    const removeLine = (itemId) => {
        setCreatePayload((p) => ({
            ...p,
            items: p.items.filter((x) => x.item_id !== itemId),
        }))
    }
    ...
    return (
        <div className="p-4 md:p-6 space-y-4">
            ...
            >
                <div>
                    <div className="text-2xl font-semibold tracking-tight">
                        Inventory Indents & Issues
                    </div>
                    <div className="text-sm text-muted-foreground">
                        NABH-friendly transfer flow — Indent → Approval → Issue → Posting
                    </div>
                </div>
                        ...
                        <div className="flex items-center gap-2">
                            <Tabs value={tab} onValueChange={setTab}>
                                <TabsList className="rounded-xl">
                                    {canIndentView && (
                                        <TabsTrigger value="indents" className="gap-2">
                        ...
                        <div className="flex flex-col md:flex-row gap-2 md:items-center">
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    className="pl-9 rounded-xl w-full md:w-[320px]"
                                />
                            </div>
                                ...
                                <div className="grid gap-2">
                                    {(indentRows || []).map((r) => (
                                        <button
                                            key={r.id}
                                            className={cx(
                                                "text-left w-full rounded-2xl border bg-white shadow-sm",
                                            ...
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="min-w-0">
                                                    <div className="font-semibold truncate">
                                                        {r.indent_number || `Indent #${r.id}`}
                                                    </div>
                                                    <div className="text-xs text-muted-foreground mt-1">
                                                        Created: {fmtIST(r.created_at)} • Indent Date:{" "}
                                                        {r.indent_date ? fmtIST(r.indent_date) : "-"}
                                                    </div>
                                                    <div className="text-xs text-muted-foreground mt-1">
                                                        From: {r.from_location?.name || r.from_location_id}{" "}
                                                        <ArrowRight className="inline w-3 h-3 mx-1" />
                                                        To: {r.to_location?.name || r.to_location_id}
                                                    </div>
                                                </div>

                                                <div className="flex flex-col items-end gap-2 shrink-0">
                                                    <Badge className={cx("rounded-xl", statusBadge(r.status))}>
                                ...
                                <div className="grid gap-2">
                                    {(issueRows || []).map((r) => (
                                        <button
                                            ...
                                            onClick={() => setIssueId(r.id)}
                                        >
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="min-w-0">
                                                    <div className="font-semibold truncate">
                                                        {r.issue_number || `Issue #${r.id}`}
                                                    </div>
                                                    <div className="text-xs text-muted-foreground mt-1">
                                                        Created: {fmtIST(r.created_at)} • Issue Date:{" "}
                                                        {r.issue_date ? fmtIST(r.issue_date) : "-"}
                                                    </div>
                                                    <div className="text-xs text-muted-foreground mt-1">
                                                        From: {r.from_location?.name || r.from_location_id}{" "}
                                                        <ArrowRight className="inline w-3 h-3 mx-1" />
            ...
            <Dialog open={filterOpen} onOpenChange={setFilterOpen}>
                <DialogContent className="max-w-[720px] rounded-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Filter list by status, locations, and date range.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="space-y-1">
                            ...
                            <Input
                                ...
                                placeholder="DRAFT / SUBMITTED / APPROVED / POSTED..."
                                className="rounded-xl"
                            />
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <Select
                                ...
                                onValueChange={(v) =>
                                    setFilters((p) => ({ ...p, from_location_id: v }))
                                }
                            >
                        ...
                        <div className="space-y-1">
                            ...
                            <Select
                                ...
                                onValueChange={(v) =>
                                    setFilters((p) => ({ ...p, to_location_id: v }))
                                }
                            >
                                <SelectTrigger className="rounded-xl">
                                    <SelectValue placeholder="All" />
            ...
            >
                <DialogContent className="max-w-[980px] rounded-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Select locations, add items, set requested quantities.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        ...
                        <div className="lg:col-span-1 space-y-3">
                            <div className="space-y-1">
                                <Label>From Location *</Label>
                                <Select
                                    value={createPayload.from_location_id}
                                    onValueChange={(v) =>
                                        setCreatePayload((p) => ({ ...p, from_location_id: v }))
                                    }
                                >
                                    <SelectTrigger className="rounded-xl">
                            ...
                            <div className="space-y-1">
                                <Label>To Location *</Label>
                                <Select
                                    value={createPayload.to_location_id}
                                    onValueChange={(v) =>
                                        setCreatePayload((p) => ({ ...p, to_location_id: v }))
                                    }
                                >
                                    <SelectTrigger className="rounded-xl">
                                        <SelectValue placeholder={catalogLoading ? "Loading..." : "Select"} />
                            ...
                            <div className="space-y-1">
                                ...
                                <Select
                                    ...
                                    onValueChange={(v) =>
                                        setCreatePayload((p) => ({ ...p, priority: v }))
                                    }
                                >
                                    <SelectTrigger className="rounded-xl">
                                        <SelectValue />
                            ...
                            <div className="space-y-1">
                                ...
                                <Input
                                    ...
                                    onChange={(e) =>
                                        setCreatePayload((p) => ({ ...p, notes: e.target.value }))
                                    }
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="space-y-1">
                                    ...
                                    <Input
                                        value={createPayload.patient_id}
                                        onChange={(e) =>
                                            setCreatePayload((p) => ({ ...p, patient_id: e.target.value }))
                                        }
                                ...
                                <div className="space-y-1">
                                    ...
                                    <Input
                                        value={createPayload.visit_id}
                                        onChange={(e) =>
                                            setCreatePayload((p) => ({ ...p, visit_id: e.target.value }))
                                        }
                                        placeholder="Optional"
                                        className="rounded-xl"
                                    />
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="space-y-1">
                                    <Label>IPD Admission ID</Label>
                                    <Input
                                        value={createPayload.ipd_admission_id}
                                        onChange={(e) =>
                                            setCreatePayload((p) => ({ ...p, ipd_admission_id: e.target.value }))
                                        }
                                        placeholder="Optional"
                                        className="rounded-xl"
                                    />
                                ...
                                <div className="space-y-1">
                                    ...
                                    <Input
                                        value={createPayload.encounter_id}
                                        onChange={(e) =>
                                            setCreatePayload((p) => ({ ...p, encounter_id: e.target.value }))
                                        }
                                        placeholder="Optional"
                                        className="rounded-xl"
                                    />
                                                        ...
                                                        <div className="mt-3 grid grid-cols-2 gap-2 items-end">
                                                            ...
                                                            <div className="flex items-center justify-between border rounded-xl px-3 py-2">
                                                                ...
                                                                <Switch
                                                                    checked={!!li.is_stat}
                                                                    onCheckedChange={(v) =>
                                                                        updateLine(li.item_id, { is_stat: v })
                                                                    }
                                                                />
                                                            </div>
                                                            ...
                                                            <div className="col-span-2 space-y-1">
                                                                ...
                                                                <Input
                                                                    value={li.remarks}
                                                                    onChange={(e) =>
                                                                        updateLine(li.item_id, { remarks: e.target.value })
                                                                    }
                                                                    placeholder="Optional"
                                                                    className="rounded-xl"
                                                                />
            ...
            <IssueDrawer
                open={!!issueId}
                issueId={issueId}
                onOpenChange={(v) => !v && setIssueId(null)}
                onChanged={() => {
                    bumpRefetch()
                    loadLists()
                }}
