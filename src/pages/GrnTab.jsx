...
const resolvePoItemId = (it) => {
  const id = it?.id ?? it?.id ?? it?.item?.id ?? null
...
function StatChip({ label, value, tone = "neutral" }) {
  ...
  return (
    <div className={["rounded-2xl border px-3 py-2", tones[tone] || tones.neutral].join(" ")}>
...
export default function GrnTab() {
        ...
        setLines(
          (g.items || []).map((it) =>
            makeLine({
              ...
              item_id: it?.id ? Number(it?.id) : null,
  ...
  const kpis = useMemo(() => {
    ...
    const mismatch = (grns || []).filter((x) => Math.abs(n(x.amount_difference || 0)) >= 0.01 && n(x.supplier_invoice_amount || 0) > 0).length
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl border border-slate-500/70 bg-gradient-to-b from-white to-slate-50/70 p-5 shadow-[0_10px_30px_rgba(2,6,23,0.06)]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-base font-semibold text-slate-900">Goods Receipt Notes</div>
                <div className="text-xs text-slate-500">Premium workflow: PO → batches → save draft → post stock</div>
              </div>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <Button variant="outline" className="rounded-2xl" onClick={() => { loadPendingPos(); loadGrns(); }}>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-4">
          ...
          <StatChip label="Invoice mismatch" value={kpis.mismatch} tone={kpis.mismatch ? "warn" : "neutral"} />
        </div>
      </div>
      ...
      <GlassCard>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="gap-1 rounded-2xl" onClick={loadPendingPos}>
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <Button variant="outline" className="rounded-2xl h-10" onClick={loadPendingPos}>
                Search
              </Button>
            </div>
      ...
      <GlassCard>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" className="h-10 rounded-2xl" onClick={loadGrns}>
              <RefreshCcw className="w-4 h-4 mr-1" />
              Refresh
            </Button>
            <Button size="sm" className="h-10 rounded-2xl gap-1" onClick={openNewGrn}>
            ...
            <div className="max-h-[420px] overflow-auto divide-y divide-slate-100">
              {grnLoading ? (
                ...
                filteredGrns.map((grn) => {
                  ...
                  const hasMismatch = Math.abs(diff) >= 0.01 && n(grn.supplier_invoice_amount || 0) > 0
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 md:mt-0 flex justify-end gap-2">
                        ...
                        {grn.status === "DRAFT" ? (
                          <Button size="sm" className="h-9 gap-1 rounded-2xl" onClick={() => openDraft(grn.id)}>
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  ...
                                  <div className="text-xs text-slate-500 truncate mt-1">
                                    {po.supplier?.name || "—"} • {po.location?.name || "—"} • {formatDate(po.order_date)}
                                  </div>
                                </div>
                      ...
                      <CardHeader className="pb-2">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              Premium cards. Each line shows Gross/Disc/Tax/Net.
                            </p>
                          </div>
                  ...
                  <div className="space-y-4 lg:sticky lg:top-4 h-fit">
                      ...
                      <CardContent className="space-y-3 text-sm">
                        ...
                        <div className="rounded-3xl border border-slate-500 bg-slate-50 p-4">
                          ...
                          <div className="mt-2 flex items-center justify-between">
                            ...
                            <div className="text-sm font-semibold text-slate-900">₹{money(totals.invoice)}</div>
                          </div>
                        </div>

                        <div className="grid grid-cols-3 gap-2">
                          <StatChip label="Lines" value={lines.length} />
                          <StatChip label="Item miss" value={totals.itemMissing} tone={totals.itemMissing ? "warn" : "neutral"} />
                    ...
                    <GlassCard className="bg-white">
                      ...
                      <CardContent className="space-y-2">
                        <Button className="w-full rounded-2xl h-11" onClick={saveDraft} disabled={savingDraft || readonly}>
