...
function getErrMsg(e) {
  const msg = e?.response?.data?.error?.msg || e?.response?.data?.message || e?.message || "Request failed"
  return String(msg)
}
...
function GlassBar({ children, className = "" }) {
  return (
    <div
      className={[
        "rounded-3xl border border-slate-200/70 bg-white/70 backdrop-blur-xl shadow-[0_10px_30px_-20px_rgba(0,0,0,0.35)]",
        className,
      ].join(" ")}
    >
      {children}
...
function SevBadge({ severity }) {
  ...
  const token = SEV_STYLES[s] || SEV_STYLES.INFO
  return (
    <Badge variant="outline" className={["rounded-full px-2.5 py-1 text-[11px] font-semibold", token.badge].join(" ")}>
      <span className={["mr-1.5 inline-block h-1.5 w-1.5 rounded-full", token.dot].join(" ")} />
      {s || "INFO"}
    </Badge>
  )
}
...
function KpiCard({ icon: Icon, title, value, sub }) {
  return (
    <Card className="group rounded-3xl border-slate-200/80 bg-white/70 backdrop-blur shadow-[0_12px_40px_-30px_rgba(0,0,0,0.45)] transition hover:-translate-y-[1px] hover:shadow-[0_18px_55px_-35px_rgba(0,0,0,0.55)]">
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          <div className="relative mt-0.5">
            <div className="absolute -inset-1 rounded-2xl bg-gradient-to-br from-indigo-500/15 via-sky-500/10 to-emerald-500/10 blur-md opacity-0 transition group-hover:opacity-100" />
            <div className="relative rounded-2xl border border-slate-200/80 bg-white p-2 shadow-sm">
              <Icon className="h-5 w-5 text-slate-800" />
            </div>
          </div>

          <div className="min-w-0">
            <div className="text-[11px] font-semibold tracking-wide text-slate-600">{title}</div>
...
function MiniTable({ columns, rows, loading, emptyText = "No data found" }) {
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white/75 backdrop-blur">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              Array.from({ length: 6 }).map((_, i) => (
                <tr key={`sk_${i}`}>
                  {columns.map((c) => (
                    <td key={`sk_${i}_${c.key}`} className="px-3 py-3">
...
function MobileCards({ rows, loading, emptyText, titleKey = "item_name", subtitle, rightTop, rightBottom, onOpen }) {
  if (loading) {
    return (
      <div className="space-y-2">
        {Array.from({ length: 6 }).map((_, i) => (
          <Card key={i} className="rounded-3xl border-slate-200/80 bg-white/70 backdrop-blur shadow-sm">
            <CardContent className="p-4">
              <div className="space-y-2">
  ...
  if (!rows?.length) {
    return (
      <div className="rounded-3xl border border-slate-200/80 bg-white/70 backdrop-blur p-6 text-center text-sm text-slate-500">
        {emptyText}
      </div>
    )
  }
    ...
    <div className="space-y-2">
      {rows.map((r, idx) => (
        <Card
          ...
          className="group cursor-pointer rounded-3xl border-slate-200/80 bg-white/70 backdrop-blur shadow-sm transition active:scale-[0.99] hover:shadow-md"
          role="button"
          tabIndex={0}
...
function SupplierCombobox({ value, onChange }) {
  ...
  const load = useCallback(async (query) => {
    ...
    try {
      ...
      const res = await listSuppliers(query || "", { is_active: true, limit: 25 }, { signal: c.signal })

      const data = res?.data ?? res
      const arr = Array.isArray(data) ? data : Array.isArray(data?.data) ? data.data : data?.rows || data?.items || []
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className="w-full justify-between rounded-2xl border-slate-200/80 bg-white/70 backdrop-blur shadow-sm hover:bg-white"
        >
      ...
      <PopoverContent className="w-[320px] p-0" align="start">
        <Command>
          <div className="flex items-center gap-2 border-b border-slate-200/70 bg-white/80 px-3 py-2 backdrop-blur">
...
function NoAccess({ required = [] }) {
  return (
    <div className="w-full p-4 md:p-6">
      <Card className="mx-auto max-w-xl rounded-3xl border-slate-200/80 bg-white/70 backdrop-blur shadow-sm">
...
function StockAlertsDashboardInner() {
  ...
  async function openBatches({ item_id, item_name, location_id }) {
    ...
    try {
      const rows = await getItemBatches(item_id, { location_id: location_id ? Number(location_id) : undefined }, { signal: controller.signal })
  ...
  async function doExport() {
    ...
    try {
      ...
      const name = `pharmacy_${reportType.toLowerCase()}_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.${ext}`
  ...
  useEffect(() => {
    ...
    loadAlerts({ reset: true, offset: 0, limit: alertsLimit })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, alertType, commonParams])

  // When alertsLimit changes, reload (only if tab is alerts)
  useEffect(() => {
    if (tab !== "alerts") return
    loadAlerts({ reset: true, offset: 0, limit: alertsLimit })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="relative w-full p-4 md:p-6">
      {/* Background (Apple-ish, colorful but subtle) */}
      ...
      {/* Header */}
      <div className="mb-4">
        <GlassBar className="p-3 md:p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <div className="relative">
                  <div className="absolute -inset-1 rounded-2xl bg-gradient-to-br from-indigo-500/20 via-sky-500/10 to-emerald-500/10 blur-md" />
                  <div className="relative rounded-2xl border border-slate-200/80 bg-white p-2 shadow-sm">
                    <Warehouse className="h-5 w-5 text-slate-900" />
                  </div>
                </div>
                <div className="min-w-0">
                  <h1 className="truncate text-lg font-semibold text-slate-900">Pharmacy Inventory • Stock & Alerts</h1>
                  <div className="mt-0.5 text-xs text-slate-600">Batch-wise stock • FEFO signals • Alerts • Export reports</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {/* ✅ Primary Refresh */}
              <Button
                className="h-10 rounded-2xl px-4 bg-gradient-to-b from-slate-900 to-slate-800 text-white shadow-sm hover:from-slate-800 hover:to-slate-700"
                onClick={() => {
                  loadLocations()
                  loadSummary()
              ...
              <Button
                variant="outline"
                className="h-10 rounded-2xl px-4 border-slate-200/80 bg-white/70 backdrop-blur shadow-sm hover:bg-white"
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs">
            <Badge variant="outline" className="rounded-full border-slate-200/80 bg-white/70 text-slate-700">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <div className="mb-4 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <TabsList className="w-max min-w-full justify-start gap-1 rounded-3xl p-1 border border-slate-200/80 bg-white/70 backdrop-blur shadow-sm">
            {[
              ["dashboard", "Dashboard"],
              ["alerts", "Alerts"],
              ["stock", "Stock (Batch-wise)"],
              ["quarantine", "Quarantine"],
            ].map(([v, label]) => (
              <TabsTrigger
                key={v}
                value={v}
                className="whitespace-nowrap rounded-2xl h-9 px-4 text-xs font-semibold transition-all
                  text-slate-600 hover:text-slate-900 hover:bg-white/70
                  data-[state=active]:bg-gradient-to-b data-[state=active]:from-indigo-600 data-[state=active]:to-indigo-500
        ...
        <TabsContent value="dashboard">
          ...
          <div className="mt-3 md:hidden">
            <Button variant="outline" className="w-full rounded-2xl border-slate-200/80 bg-white/70 backdrop-blur hover:bg-white" onClick={() => setShowMoreKpis((s) => !s)}>
              {showMoreKpis ? "Show less" : "Show more insights"}
            </Button>
          </div>

          <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
            <Card className="rounded-3xl border-slate-200/80 bg-white/70 backdrop-blur shadow-sm">
              <CardHeader className="pb-2">
              ...
              <CardContent className="pt-0">
                ...
                <div className="md:hidden">
                  <MobileCards
                    ...
                    rightTop={(r) => <Badge variant="outline" className="rounded-full">{fmtMoney(r.stock_value_purchase)}</Badge>}
                ...
                <div className="hidden md:block">
                  <MiniTable
                    ...
                    columns={[
                      ...
                      { key: "type", header: "Type", render: (r) => <Badge className="rounded-full bg-slate-900 text-white">{r.type}</Badge> },
                      {
                        key: "item_name",
                        header: "Item",
                        render: (r) => (
                      ...
                      { key: "on_hand_qty", header: "Qty", render: (r) => fmtNum(r.on_hand_qty) },
                      {
                        ...
                        render: (r) => (
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-8 rounded-2xl border-slate-200/80 bg-white/70 backdrop-blur hover:bg-white"
                  ...
                  <MobileCards
                    ...
                    onOpen={(r) => {
                      openDetail(
                        ...
                        <Button
                          className="w-full rounded-2xl border-slate-200/80 bg-white/70 backdrop-blur hover:bg-white"
                          variant="outline"
                          onClick={() => openBatches({ item_id: r.item_id, item_name: r.item_name, location_id: r.location_id })}
                        >
                          <Eye className="mr-2 h-4 w-4" />
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Select value={alertType} onValueChange={(v) => setAlertType(v)}>
                    <SelectTrigger className="h-9 w-[220px] rounded-2xl bg-white/70 backdrop-blur">
                  ...
                  <Select value={String(alertsLimit)} onValueChange={(v) => setAlertsLimit(Number(v))}>
                    <SelectTrigger className="h-9 w-[120px] rounded-2xl bg-white/70 backdrop-blur">
                  ...
                  <Button
                    variant="outline"
                    className="h-9 rounded-2xl border-slate-200/80 bg-white/70 backdrop-blur hover:bg-white"
                    onClick={() => loadAlerts({ reset: true, offset: 0, limit: alertsLimit })}
                  >
                    <RefreshCcw className="mr-2 h-4 w-4" />
                    Reload
                  </Button>
            ...
            <CardContent className="pt-0">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                    className="rounded-2xl border-slate-200/80 bg-white/70 backdrop-blur hover:bg-white"
                ...
                <MiniTable
                  ...
                  columns={[
                    ...
                    },
                    {
                      key: "message",
                      header: "Message",
                      render: (r) => <div className="min-w-[220px] text-xs text-slate-700">{r.message}</div>,
                    },
                    {
                      key: "action",
                      header: "Actions",
          ...
          <Card className="rounded-3xl border-slate-200/80 bg-white/70 backdrop-blur shadow-sm">
            <CardHeader className="pb-2">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="relative">
                    ...
                    <Input
                      className="h-9 w-[260px] rounded-2xl pl-9 bg-white/70 backdrop-blur"
                  ...
                  <Button
                    variant="outline"
                    className="h-9 rounded-2xl border-slate-200/80 bg-white/70 backdrop-blur hover:bg-white"
                    onClick={() => loadStock({ reset: true, offset: 0 })}
                  >
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-600">
                <span className="rounded-full border border-slate-200/80 bg-white/70 px-3 py-1 backdrop-blur">
                  Purchase Value: <span className="font-semibold text-slate-900">{fmtMoney(stockResp?.totals?.value_purchase)}</span>
                </span>
                <span className="rounded-full border border-slate-200/80 bg-white/70 px-3 py-1 backdrop-blur">
            ...
            <CardContent className="pt-0">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <Pager total={stockResp?.total || 0} limit={stockLimit} offset={stockOffset} onChange={({ limit, offset }) => loadStock({ limit, offset })} />
              ...
              <div className="hidden md:block">
                <MiniTable
                  ...
                  columns={[
                    ...
                    { key: "qty", header: "Qty", render: (r) => <span className="font-semibold text-slate-900">{fmtNum(r.qty)}</span> },
              ...
              <div className="md:hidden">
                <MobileCards
                  ...
                  titleKey="item_name"
                  subtitle={(r) => `${r.location_name || r.location_code || "—"} • Batch: ${r.batch_no || "—"} • Exp: ${fmtDate(r.expiry_date)}`}
                  rightTop={(r) => <Badge variant="outline" className="rounded-full">{fmtNum(r.qty)}</Badge>}
        ...
        <TabsContent value="quarantine">
          <Card className="rounded-3xl border-slate-200/80 bg-white/70 backdrop-blur shadow-sm">
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button
                    variant="outline"
                    className="h-9 rounded-2xl border-slate-200/80 bg-white/70 backdrop-blur hover:bg-white"
                    onClick={() => loadQuarantine({ reset: true, offset: 0 })}
                  >
                    <Search className="mr-2 h-4 w-4" />
                    Search
                  </Button>
            ...
            <CardContent className="pt-0">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <Pager total={quarResp?.total || 0} limit={quarLimit} offset={quarOffset} onChange={({ limit, offset }) => loadQuarantine({ limit, offset })} />
      ...
      <Dialog open={filtersOpen} onOpenChange={setFiltersOpen}>
        <DialogContent className="w-[calc(100vw-1.25rem)] sm:max-w-2xl rounded-3xl p-0 overflow-hidden">
          <div className="border-b border-slate-200/70 bg-white/80 backdrop-blur px-5 py-4">
            <DialogHeader>
          ...
          <div className="max-h-[75vh] overflow-y-auto px-5 py-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-12">
              ...
              <div className="md:col-span-4">
                ...
                <Select value={draft.locationId} onValueChange={(v) => setDraft((s) => ({ ...s, locationId: v }))}>
                  <SelectTrigger className="rounded-2xl bg-white/70 backdrop-blur">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="md:col-span-4">
                ...
                <Select value={draft.itemType} onValueChange={(v) => setDraft((s) => ({ ...s, itemType: v }))}>
                  <SelectTrigger className="rounded-2xl bg-white/70 backdrop-blur">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="md:col-span-4">
                ...
                <Select value={draft.scheduleCode} onValueChange={(v) => setDraft((s) => ({ ...s, scheduleCode: v }))}>
                  <SelectTrigger className="rounded-2xl bg-white/70 backdrop-blur">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="md:col-span-12">
                ...
                <SupplierCombobox value={draft.supplierId} onChange={(v) => setDraft((s) => ({ ...s, supplierId: v }))} />
          ...
          <div className="border-t border-slate-200/70 bg-white/80 backdrop-blur px-5 py-4">
            <div className="flex flex-col-reverse gap-2 md:flex-row md:justify-between">
              <Button variant="outline" className="rounded-2xl" onClick={resetDraft}>
                Reset
              </Button>
              <div className="flex gap-2">
                <Button variant="outline" className="rounded-2xl" onClick={() => setFiltersOpen(false)}>
                  Cancel
                </Button>
                <Button className="rounded-2xl bg-gradient-to-b from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-500" onClick={applyFilters}>
                  Apply
                </Button>
              </div>
            </div>
      ...
      <Dialog open={exportOpen} onOpenChange={setExportOpen}>
        <DialogContent className="w-[calc(100vw-1.25rem)] sm:max-w-lg rounded-3xl p-0 overflow-hidden">
          <div className="border-b border-slate-200/70 bg-white/80 backdrop-blur px-5 py-4">
            <DialogHeader>
            ...
            <div className="grid grid-cols-1 gap-3">
              <div>
                ...
                <Select value={reportType} onValueChange={setReportType}>
                  <SelectTrigger className="h-9 rounded-2xl bg-white/70 backdrop-blur">
              ...
              <Button
                className="h-10 rounded-2xl bg-gradient-to-b from-slate-900 to-slate-800 text-white hover:from-slate-800 hover:to-slate-700"
                disabled={exporting}
                onClick={doExport}
              >
                <Download className="mr-2 h-4 w-4" />
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="w-[calc(100vw-1.25rem)] sm:max-w-lg rounded-3xl p-0 overflow-hidden">
          ...
          <div className="px-5 py-4">
            <div className="rounded-3xl border border-slate-200/80 bg-white/70 backdrop-blur px-3">
      ...
      <Dialog open={batchesOpen} onOpenChange={setBatchesOpen}>
        <DialogContent className="w-[calc(100vw-1.25rem)] sm:max-w-3xl rounded-3xl p-0 overflow-hidden">
          <div className="border-b border-slate-200/70 bg-white/80 backdrop-blur px-5 py-4">
            <DialogHeader>
              <DialogTitle className="text-base">Item Batches (Batch-wise prices)</DialogTitle>
              <DialogDescription>
                {batchesItem?.item_name ? <span className="font-medium text-slate-900">{batchesItem.item_name}</span> : "—"}
          ...
          <div className="max-h-[75vh] overflow-y-auto px-5 py-4">
            <MiniTable
              ...
              columns={[
                { key: "batch_no", header: "Batch", render: (r) => <span className="font-mono text-xs">{r.batch_no}</span> },
                { key: "expiry_date", header: "Expiry", render: (r) => fmtDate(r.expiry_date) },
                { key: "current_qty", header: "Qty", render: (r) => <span className="font-semibold text-slate-900">{fmtNum(r.current_qty)}</span> },
                { key: "unit_cost", header: "Unit Cost", render: (r) => fmtMoney(r.unit_cost) },
                { key: "mrp", header: "MRP", render: (r) => fmtMoney(r.mrp) },
