// FILE: src/pharmacy/pages/procurement/GrnsPage.jsx
import React, { useEffect, useState } from "react"
import { Link } from "react-router-dom"
import { Plus, Search } from "lucide-react"
import { toast } from "sonner"
import { phListGRNs } from "../../../api/pharmacy_new"
...
export default function GrnsPage() {
    ...
    const load = async () => {
        setLoading(true)
        try {
            const data = await phListGRNs({ store_id: storeId || undefined, q: q || undefined, limit: 200, offset: 0 })
            setRows(data?.items || [])
        } catch (e) {
            toast.error(e.message || "Failed to load GRNs")
            setRows([])
        } finally {
            setLoading(false)
        }
        ...
        <div className="space-y-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h2 className="text-sm font-semibold text-slate-900">GRN (Goods Receipt)</h2>
                        <p className="text-[12px] text-slate-500">Approve → Post to create batches, ledger IN, and stock balances.</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="relative w-full md:w-80">
                            ...
                            <input
                                ...
                                placeholder="Search supplier / invoice / GRN no..."
                            />
                        </div>
                        <Link
                            to="/pharmacy/proc/grns/new"
            ...
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-slate-100">
                    {rows.map((r) => (
                        <Link
                            key={r.id}
                            to={`/pharmacy/proc/grns/${r.id}`}
                            className="block hover:bg-slate-50/60"
                        >
                            <div className="p-4 flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                    <p className="text-sm font-semibold text-slate-900 truncate">GRN #{r.id}</p>
                                    <p className="text-[12px] text-slate-500 truncate">
                                        Supplier: {r.supplier_name || r.supplier_id || "—"} • Date: {fmtDate(r.grn_date || r.created_at)}
                                    </p>
                                </div>
                                <TonePill s={r.status} />
                            </div>
                    ...
                    {!rows.length && !loading ? (
                        <div className="p-8 text-center text-slate-500">No GRNs yet. Create your first GRN.</div>
                    ) : null}
                </div>
            </div>
        </div>
