...
import { listLabTests, createLisOrder, listLisOrders, fetchLisReportPdf } from "./lab";
import { listRisTests, createRisOrder, listRisOrders } from "./ris";
import {
    searchPharmacyItems,
    createPharmacyPrescription,
    listPharmacyPrescriptions,
...
import { listOtProcedures, listOtSchedules, createOtSchedule } from "./ot";
...
export const listVisitFollowups = async (visitId) => {
    const res = await API.get(`/opd/visits/${visitId}/followups`);
    return res.data;
};
...
export async function listLabOrdersForContext({ patientId, contextType, contextId, limit = 10 }) {
    if (!patientId) return [];
    const ctx = normalizeCtx(contextType);
...
export async function listRadiologyOrdersForContext({ patientId, contextType, contextId, limit = 10 }) {
    if (!patientId) return [];
    const ctx = normalizeCtx(contextType);
...
export async function createPharmacyPrescriptionFromContext({
    ...
    const ctx = normalizeCtx(contextType);
    if (!patientId || !ctx || !contextId) throw new Error("Missing patient / context for prescription");
    ...
    if (!mappedLines.length) throw new Error("No medicines found in inventory master");
...
export async function listPharmacyPrescriptionsForContext({ patientId, contextType, contextId, limit = 10 }) {
    if (!patientId) return [];
    const ctx = normalizeCtx(contextType);
    const type = ctx === "ipd" ? "IPD" : "OPD";
...
export function createOtScheduleFromContext({
    ...
    if (!date || !plannedStartTime) throw new Error("OT date and start time are required");
    if (!surgeonUserId) throw new Error("Surgeon is required for OT schedule");

    const procName = (procedureName || "").trim();
    if (!procName) throw new Error("Procedure name is required for OT schedule");
    ...
    const payload = {
        ...
        patient_id: patientId ? Number(patientId) : null,
        admission_id: ctx === "ipd" && admissionId ? Number(admissionId) : null,

        bed_id: bedId ? Number(bedId) : null,

        surgeon_user_id: Number(surgeonUserId),
        anaesthetist_user_id: anaesthetistUserId ? Number(anaesthetistUserId) : null,
        ...
        primary_procedure_id: primaryProcedureId ? Number(primaryProcedureId) : null,
        additional_procedure_ids: Array.isArray(additionalProcedureIds)
            ? additionalProcedureIds.map((id) => Number(id))
            : [],
    };
...
export async function listOtSchedulesForContext({ patientId, admissionId, limit = 10 }) {
    if (!patientId) return [];
    const res = await listOtSchedules({ patient_id: patientId });
    const rows = Array.isArray(res?.data) ? res.data : [];

    const filtered = rows.filter((s) => {
        if (admissionId && s.admission_id && s.admission_id !== admissionId) return false;
        return true;
    });
...
export async function downloadRxPdf(rxId) {
    return fetchPharmacyPrescriptionPdf(rxId);
}
