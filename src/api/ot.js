...
export const scheduleOtOrder = (id, payload) =>
    API.post(`/ot/orders/${id}/schedule`, payload)
export const updateOtStatus = (id, status) =>
    API.post(`/ot/orders/${id}/status`, { status })
export const uploadOtAttachment = (id, file, note = '') => {
    const fd = new FormData()
    fd.append('file', file)
    if (note) fd.append('note', note)
    return API.post(`/ot/orders/${id}/upload`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
    })
}
export const addOtAttachmentLink = (id, payload) =>
    API.post(`/ot/orders/${id}/attachments`, payload)

// --- NEW: History per patient (for OT History tab)
export const getOtHistory = (patientId) =>
    API.get('/ot/history', { params: { patient_id: patientId } })
