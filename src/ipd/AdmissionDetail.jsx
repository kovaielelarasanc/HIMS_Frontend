// src/ipd/AdmissionDetail.jsx
import { useEffect, useMemo, useState } from 'react'
import { useParams, useLocation, Link } from 'react-router-dom'
import { useCan } from '../hooks/usePerm'
import {
    getAdmission, updateAdmission, cancelAdmission, transferBed, listBeds,
    getPatient
} from '../api/ipd'
...
export default function AdmissionDetail() {
    ...
    const load = async () => {
        ...
        try {
            const [{ data: a }, b] = await Promise.all([getAdmission(Number(id)), listBeds()])
            setAdmission(a || admissionFromList || null)
            setBeds(b.data || [])
            try { const { data: p } = await getPatient(a.patient_id); setPatient(p) } catch { /* no-op */ }
        } catch (e) {
    ...
    useEffect(() => {
        ...
        setEdit({
            ...
            expected_discharge_at: admission.expected_discharge_at ? new Date(admission.expected_discharge_at).toISOString().slice(0, 16) : '',
    ...
    const saveEdit = async (e) => {
        ...
        try {
            const payload = {
                practitioner_user_id: edit.practitioner_user_id ? Number(edit.practitioner_user_id) : undefined,
                department_id: edit.department_id ? Number(edit.department_id) : undefined,
    ...
    const doCancel = async () => {
        ...
        try { await cancelAdmission(admission.id); window.location.assign('/ipd/tracking') }
        catch (e1) { setError(e1?.response?.data?.detail || 'Cancel failed') }
    }

    const doTransfer = async (newBedId) => {
        try { await transferBed(admission.id, { to_bed_id: Number(newBedId), reason: 'Transfer' }); await load() }
        catch (e1) { setError(e1?.response?.data?.detail || 'Transfer failed') }
    }
    ...
    const Header = () => (
        <div className="rounded-xl border bg-white p-3 text-sm grid md:grid-cols-4 gap-2">
            <div><span className="text-gray-500">Admission:</span> {admissionCode(admission.id)}</div>
            <div><span className="text-gray-500">Patient:</span> {patient?.uhid || `P-${admission.patient_id}`}</div>
            <div><span className="text-gray-500">Bed:</span> {admission.current_bed_id ? (beds.find(b => b.id === admission.current_bed_id)?.code || '—') : '—'}</div>
            <div><span className="text-gray-500">Status:</span> {admission.status}</div>
        </div>
    )
        ...
        return (
            <div className="rounded-xl border bg-white">
                <div className="border-b px-3 py-2 flex flex-wrap gap-2">
                    {TABS.map(t => {
                        const canWriteThis = useCan(t.writePerm)
                        ...
                        return (
                            <button
                                ...
                                className={[
                                    'relative px-3 py-1 rounded-lg text-sm transition',
                                    isActive ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200',
                                ].join(' ')}
                                title={canWriteThis ? '' : 'View-only'}
                            >
                                {t.label}
                                {!canWriteThis && (
                                    <span className="ml-2 text-[10px] rounded bg-gray-200 px-1.5 py-0.5 text-gray-700 align-middle">
                                        view-only
                                    </span>
                                )}
                            </button>
                ...
                <div className="p-3">
                    {(() => {
                        ...
                        const TabEl = tab.el
                        const canWriteThis = useCan(tab.writePerm)
                        // Pass canWrite down so each tab can hide its write UI
                        return <TabEl admissionId={admission.id} canWrite={canWriteThis} />
                    })()}
                </div>
            </div>
        )
                ...
                <div className="rounded-xl border bg-white p-3 text-sm grid md:grid-cols-2 gap-4">
                    <form onSubmit={saveEdit} className="space-y-3">
                        ...
                        <DeptRoleUserPicker
                            ...
                            onChange={(userId, ctx) => {
                                setEdit(s => ({ ...s, practitioner_user_id: userId || '', department_id: ctx?.department_id || s.department_id }))
                            }}
                        ...
                        <div className="grid gap-3 md:grid-cols-2">
                            <div>
                                ...
                                <input type="datetime-local" className="input" value={edit.expected_discharge_at} onChange={e => setEdit(s => ({ ...s, expected_discharge_at: e.target.value }))} />
                            </div>
                            <div>
                                <label className="text-xs text-gray-500">Payor type</label>
                                <select className="input" value={edit.payor_type} onChange={e => setEdit(s => ({ ...s, payor_type: e.target.value }))}>
                                    {['cash', 'insurance', 'tpa'].map(x => <option key={x} value={x}>{x}</option>)}
                                </select>
                            </div>
                            <input className="input" placeholder="Insurer name" value={edit.insurer_name} onChange={e => setEdit(s => ({ ...s, insurer_name: e.target.value }))} />
                            <input className="input" placeholder="Policy number" value={edit.policy_number} onChange={e => setEdit(s => ({ ...s, policy_number: e.target.value }))} />
                            <input className="input md:col-span-2" placeholder="Preliminary diagnosis" value={edit.preliminary_diagnosis} onChange={e => setEdit(s => ({ ...s, preliminary_diagnosis: e.target.value }))} />
                            <input className="input" placeholder="History" value={edit.history} onChange={e => setEdit(s => ({ ...s, history: e.target.value }))} />
                            <input className="input" placeholder="Care plan" value={edit.care_plan} onChange={e => setEdit(s => ({ ...s, care_plan: e.target.value }))} />
                        </div>

                        <div className="flex items-center gap-2">
                            <button className="btn">Save</button>
                            <button type="button" className="btn bg-rose-600 hover:bg-rose-700" onClick={doCancel}>Cancel Admission</button>
                        </div>

                        {error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-2 text-rose-700 text-sm">{error}</div>}
                    </form>

                    <div className="space-y-2">
                        <div className="text-sm font-medium">Transfer Bed</div>
                        <WardRoomBedPicker value={''} onChange={doTransfer} />
                        <div className="text-xs text-gray-500">Pick a target bed (vacant) to transfer.</div>
                    </div>
                </div>
            )}
