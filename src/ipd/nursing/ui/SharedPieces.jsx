...
export function StatusPill({ status }) {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${toneClass(tone)}`}>
            {status || '—'}
        </span>
    )
}
...
export function StickyActionBar({ canWrite, onDraft, onSubmit, submitting, draftLabel = 'Save draft', submitLabel = 'Save' }) {
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden">
            <div className="mx-auto max-w-[980px] px-4 pb-4">
                <div className="rounded-2xl border bg-white/95 backdrop-blur shadow-sm p-2 flex items-center gap-2">
                    <Button variant="outline" className="h-11 rounded-xl flex-1" onClick={onDraft} disabled={submitting}>
                        {draftLabel}
                    </Button>
