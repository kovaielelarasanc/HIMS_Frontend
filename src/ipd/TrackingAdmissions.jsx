 ...
 * - finally fallback to legacy "ADM-000123"
 */
...
export default function TrackingAdmissions() {
  ...
  const pmapRef = useRef({}) // ✅ cache without dependency loop

  useEffect(() => {
    pmapRef.current = pmap
  }, [pmap])
  ...
  const loadData = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      // ✅ IMPORTANT: do not reset page on silent auto-refresh
      if (!silent) setPage(1)
    } catch (e) {
  ...
  const ensurePatientUhids = useCallback(async (pids) => {
    ...
    const current = pmapRef.current || {}
    const missing = uniq.filter((pid) => !current[pid])
                ...
                <div className="mt-3 flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                      Monitor currently admitted patients with bed position and quick open.
                    </p>
                ...
                <div className="grid w-full gap-3 md:grid-cols-[2fr,1.2fr]">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-slate-500">
                      Tip: Type IP code like <span className="font-semibold">NHIP16122025000001</span> or bed code like{' '}
            ...
            {!loading && pageRows.length > 0 && (
              <>
                          ...
                          <div className="mt-3 space-y-2 text-[12px] text-slate-700">
                            ...
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                {/* ✅ IST everywhere */}
                                <div className="text-[12px]">{formatIST(r.admitted_at)}</div>
                              </div>
                            </div>
                          </div>
                        </motion.div>
                ...
                <div className="mt-3 flex items-center justify-between gap-2">
                  <button type="button" onClick={handlePrev} disabled={disablePrev} className={UI.chipBtn}>
                  ...
                  <button type="button" onClick={handleNext} disabled={disableNext} className={UI.chipBtn}>
                    Next
