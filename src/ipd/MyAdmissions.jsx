...
export default function MyAdmissions() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      admissions.sort((x, y) => {
        const dx = parseApiDateUTC(x.admitted_at)?.getTime?.() || 0
        const dy = parseApiDateUTC(y.admitted_at)?.getTime?.() || 0
        return dy - dx
      })
      ...
      await Promise.all(
        ids.map(async (pid) => {
          try {
            ...
            // keep it UHID-focused (as your table label says Patient (UHID))
      ...
      setError(e?.response?.data?.detail || 'Failed to load your admissions. Please try again.')
      setRows([])
      setBedsById({})
      setPmap({})
    } finally {
  ...
  if (!me?.id) {
    return (
      <div className="min-h-screen bg-slate-50 px-4 py-6 text-sm text-slate-700 md:px-6">
        <div className="mx-auto max-w-3xl rounded-2xl border border-black/10 bg-white p-4 shadow-sm">
          You are not logged in or your user information is missing. Please log in again to view your admissions.
        </div>
      </div>
    )
  }

  const displayName = me?.full_name || me?.name || me?.username || me?.email || 'you'
    ...
    <div className="min-h-[calc(100vh-4rem)] w-full bg-gradient-to-b from-slate-50 via-white to-slate-50 px-4 py-4 text-black md:px-6 md:py-6">
      <div className="mx-auto max-w-6xl space-y-3">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-black/10 bg-white/75 backdrop-blur-xl shadow-[0_14px_40px_rgba(2,6,23,0.10)]">
          <div className="absolute inset-0 opacity-60 bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.18),_transparent_55%)]" />
          <div className="relative p-4 sm:p-5">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] sm:text-xs text-slate-600">
                      Active IPD admissions where <span className="font-semibold">{displayName}</span> is primary doctor.
                    </p>
                  </div>
                </div>

                {/* Compact info line (mobile-friendly) */}
                <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px]">
                  <span className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/80 px-3 py-1 font-semibold text-slate-700">
                    ...
                    Assigned <span className="tabular-nums text-slate-900">{filteredRows.length}</span>
                  </span>
                  ...
                  {lastSyncAt && (
                    <span className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/80 px-3 py-1 font-semibold text-slate-700">
                      ...
                      Synced <span className="tabular-nums text-slate-900">{lastSyncAt.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span>
                    </span>
                  )}
                </div>
              </div>
            ...
            <p className="mt-2 text-[11px] text-slate-500">
              Tip: Type <span className="font-semibold">ADM-000123</span>, UHID or bed code to quickly find a patient.
            </p>
          </div>
        </div>
                  ...
                  {pageRows.map((r) => {
                    ...
                    return (
                      <tr className="border-t border-black/5 hover:bg-black/[0.02]" key={r.id}>
                        ...
                        <td className="px-4 py-3 align-middle tabular-nums font-semibold text-slate-900">
                        ...
                        <td className="px-4 py-3 align-middle text-right">
                          <Link
                            ...
                            state={{ admission: r }}
                            className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-slate-800"
                          >
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Link
                        to={`/ipd/admission/${r.id}`}
                        state={{ admission: r }}
                        className="inline-flex items-center gap-1.5 rounded-full border border-black/10 bg-slate-900 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm transition hover:bg-slate-800"
                      >
                        Open <ArrowUpRight className="h-4 w-4" />
                      </Link>
                    </div>
