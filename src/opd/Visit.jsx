...
const UI = {
    ...
    softBox:
        'rounded-3xl border border-black/50 bg-white/70 backdrop-blur px-4 py-3',
}
...
// ✅ Templates + Macros (edit these anytime)
const TEMPLATES = {
    // HISTORY
    hpi: `Onset:\nDuration:\nCourse/Progression:\nCharacter:\nRadiation:\nAggravating factors:\nRelieving factors:\nAssociated symptoms:\nPrevious episodes:\nKey negatives:\n`,
    ros: `General:\nRespiratory:\nCardiovascular:\nGI:\nGU:\nCNS:\nMusculoskeletal:\nSkin:\nEndocrine:\nHematologic:\n`,
        ...
        <div className="space-y-2">
            <div className="flex items-end justify-between gap-2">
                <div>
                    ...
                    {hint ? <div className="text-[12px] text-slate-600 mt-0.5">{hint}</div> : null}
                </div>
...
export default function Visit({ currentUser }) {
    ...
    const quickOrdersOpNumber = useMemo(() => (data ? data.episode_id : undefined), [data])

    // Prescription helpers
    const addRxRow = () => {
        setRx((r) => ({
            ...r,
            items: [...(r.items || []), { drug_name: '', strength: '', frequency: '', duration_days: 0, quantity: 0, unit_price: 0 }],
        }))
    }
    const removeRxRow = (idx) => setRx((r) => ({ ...r, items: (r.items || []).filter((_, i) => i !== idx) }))
    const onRxItem = (idx, key, val) =>
        setRx((r) => ({ ...r, items: (r.items || []).map((it, i) => (i === idx ? { ...it, [key]: val } : it)) }))
    ...
    // ✅ Print PDF (server generated)
    const printPdf = async () => {
        try {
            setPdfing(true)
            if (hasChanges) {
                toast.message('Saving before print…')
                await save()
            }
                    ...
                    <AnimatePresence mode="wait" initial={false}>
                                    ...
                                    <CardContent className="pt-4 space-y-4">
                                        <div className={cx(UI.softBox, 'flex flex-wrap items-center justify-between gap-2')}>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                <button className={UI.chipBtn} onClick={() => applyText('presenting_illness', TEMPLATES.hpi)}>HPI template</button>
                                                <button className={UI.chipBtn} onClick={() => applyText('review_of_systems', TEMPLATES.ros)}>ROS template</button>
                                                <button className={UI.chipBtn} onClick={() => applyText('medical_history', TEMPLATES.pmh)}>PMH template</button>
                                            </div>
                                        </div>

                                        <div className="grid gap-3 md:grid-cols-2">
                                            <Field label="Chief complaint">
                                                <Textarea value={form.chief_complaint} onChange={(e) => onField('chief_complaint', e.target.value)} className={UI.textarea} />
                                            </Field>

                                            <Field label="Presenting illness (HPI)">
                                                <Textarea value={form.presenting_illness} onChange={(e) => onField('presenting_illness', e.target.value)} className={UI.textarea} />
                                            </Field>
                                        </div>

                                        <div className="grid gap-3 md:grid-cols-2">
                                            <Field label="Symptoms">
                                                <Textarea value={form.symptoms} onChange={(e) => onField('symptoms', e.target.value)} className={UI.textarea} />
                                            </Field>

                                            <Field label="Review of Systems (ROS)">
                                                <Textarea value={form.review_of_systems} onChange={(e) => onField('review_of_systems', e.target.value)} className={UI.textarea} />
                                            </Field>
                                        </div>
                                        ...
                                        <div className="grid gap-3 md:grid-cols-2">
                                            <Field label="Past medical history (PMH)">
                                                <Textarea value={form.medical_history} onChange={(e) => onField('medical_history', e.target.value)} className={UI.textarea} />
                                            </Field>

                                            <Field label="Past surgical history (PSH)">
                                                <Textarea value={form.surgical_history} onChange={(e) => onField('surgical_history', e.target.value)} className={UI.textarea} />
                                            </Field>
                                        </div>

                                        <div className="grid gap-3 md:grid-cols-2">
                                            <Field label="Medication history">
                                                <Textarea value={form.medication_history} onChange={(e) => onField('medication_history', e.target.value)} className={UI.textarea} />
                                            </Field>

                                            <Field label="Drug allergy / allergies">
                                                <Textarea value={form.drug_allergy} onChange={(e) => onField('drug_allergy', e.target.value)} className={UI.textarea} />
                                            </Field>
                                        </div>

                                        <div className="grid gap-3 md:grid-cols-2">
                                            <Field label="Family history">
                                                <Textarea value={form.family_history} onChange={(e) => onField('family_history', e.target.value)} className={UI.textarea} />
                                            </Field>

                                            <Field label="Personal / social history">
                                                <Textarea value={form.personal_history} onChange={(e) => onField('personal_history', e.target.value)} className={UI.textarea} />
                                            </Field>
                                        </div>
                                        ...
                                        <div className="grid gap-3 md:grid-cols-2">
                                            <Field label="Menstrual history (if applicable)">
                                                <Textarea value={form.menstrual_history} onChange={(e) => onField('menstrual_history', e.target.value)} className={UI.textarea} />
                                            </Field>

                                            <Field label="Obstetric history (if applicable)">
                                                <Textarea value={form.obstetric_history} onChange={(e) => onField('obstetric_history', e.target.value)} className={UI.textarea} />
                                            </Field>
                                        </div>

                                        <Field label="Immunization history (optional)">
                                            <Textarea value={form.immunization_history} onChange={(e) => onField('immunization_history', e.target.value)} className={UI.textarea} />
                                        </Field>
                                        ...
                                        <div className={cx(UI.softBox, 'flex items-center justify-between gap-2')}>
                                            ...
                                            <Button type="button" className="h-11 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-semibold" onClick={save} disabled={saving || !hasChanges}>
                                ...
                                <Card className={cx(UI.glass)}>
                                    ...
                                    <CardContent className="pt-4 space-y-4">
                                        <div className={cx(UI.softBox, 'flex flex-wrap items-center justify-between gap-2')}>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                <button className={UI.chipBtn} onClick={() => applyText('general_examination', TEMPLATES.exam_normal_general)}>Normal General</button>
                                                <button className={UI.chipBtn} onClick={() => applyText('systemic_examination', TEMPLATES.exam_normal_systemic)}>Normal Systemic</button>
                                                <button className={UI.chipBtn} onClick={() => applyMulti({
                                                    general_examination: TEMPLATES.exam_normal_general,
                                                    systemic_examination: TEMPLATES.exam_normal_systemic,
                                                })}>
                                                    Normal Exam (All)
                                                </button>
                                                <button className={UI.chipBtn} onClick={() => applyText('systemic_examination', TEMPLATES.exam_cvs)}>CVS</button>
                                                <button className={UI.chipBtn} onClick={() => applyText('systemic_examination', TEMPLATES.exam_rs)}>RS</button>
                                            </div>
                                        </div>

                                        <Field label="General examination">
                                            <Textarea value={form.general_examination} onChange={(e) => onField('general_examination', e.target.value)} className={UI.textarea} />
                                        </Field>

                                        <div className="grid gap-3 md:grid-cols-2">
                                            <Field label="Systemic examination">
                                                <Textarea value={form.systemic_examination} onChange={(e) => onField('systemic_examination', e.target.value)} className={UI.textarea} />
                                            </Field>

                                            <Field label="Local examination (optional)">
                                                <Textarea value={form.local_examination} onChange={(e) => onField('local_examination', e.target.value)} className={UI.textarea} />
                                            </Field>
                                        </div>
                                        ...
                                        <Field label="SOAP Objective (Legacy)">
                                            <Textarea value={form.soap_objective} onChange={(e) => onField('soap_objective', e.target.value)} className={UI.textarea} />
                                        </Field>
                                    ...
                                    <CardContent className="pt-4 space-y-4">
                                        <div className={cx(UI.softBox, 'flex flex-wrap items-center justify-between gap-2')}>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                ...
                                                <button className={UI.chipBtn} onClick={() => applyText('diagnosis_codes', TEMPLATES.icd_hint)}>ICD hint</button>
                                            </div>
                                        </div>

                                        <div className="grid gap-3 md:grid-cols-2">
                                            <Field label="Provisional diagnosis">
                                                <Textarea value={form.provisional_diagnosis} onChange={(e) => onField('provisional_diagnosis', e.target.value)} className={UI.textarea} />
                                            </Field>

                                            <Field label="Differential diagnosis">
                                                <Textarea value={form.differential_diagnosis} onChange={(e) => onField('differential_diagnosis', e.target.value)} className={UI.textarea} />
                                            </Field>
                                        </div>

                                        <Field label="Final diagnosis">
                                            <Textarea value={form.final_diagnosis} onChange={(e) => onField('final_diagnosis', e.target.value)} className={UI.textarea} />
                                        </Field>

                                        <Field label="Diagnosis codes (ICD)" hint="Comma-separated (example: I10,E11.9)">
                                            <Input value={form.diagnosis_codes} onChange={(e) => onField('diagnosis_codes', e.target.value)} placeholder="I10,E11.9" className={UI.input} />
                                        </Field>

                                        <Field label="SOAP Assessment (Legacy)">
                                            <Textarea value={form.soap_assessment} onChange={(e) => onField('soap_assessment', e.target.value)} className={UI.textarea} />
                                        </Field>
                                    ...
                                    <CardContent className="pt-4 space-y-4">
                                        <div className={cx(UI.softBox, 'flex flex-wrap items-center justify-between gap-2')}>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                ...
                                                <button className={UI.chipBtn} onClick={() => applyText('advice', TEMPLATES.advice_general)}>General advice</button>
                                            </div>
                                        </div>
                                        ...
                                        <div className={cx(UI.softBox, 'space-y-3')}>
                                            ...
                                            <div className="grid gap-3 md:grid-cols-[1fr,2fr,auto] md:items-end">
                                                <div className="space-y-2">
                                                    ...
                                                    <Input type="date" value={fuDate} onChange={(e) => setFuDate(e.target.value)} className={UI.input} />
                                                </div>

                                                <div className="space-y-2">
                                                    <div className={UI.label}>Note</div>
                                                    <Input value={fuNote} onChange={(e) => setFuNote(e.target.value)} placeholder="Review in 2 weeks / discuss reports…" className={UI.input} />
                                                </div>

                                                <Button type="button" className="h-11 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-semibold px-6" onClick={createFu} disabled={fuSaving}>
                            ...
                            >
                                <Card className={cx(UI.glass)}>
                                    ...
                                    <CardContent className="pt-4 space-y-3">
                                        <div className="flex flex-wrap gap-2">
                                            <span className={UI.chip}><Microscope className="h-3.5 w-3.5" /> Lab</span>
                                            <span className={UI.chip}><ScanLine className="h-3.5 w-3.5" /> Radiology</span>
                                            <span className={UI.chip}><Pill className="h-3.5 w-3.5" /> Pharmacy</span>
                                        </div>
                        ...
                        {activeTab === 'rx' && (
                            ...
                            >
                                <Card className={cx(UI.glass)}>
                                    <CardHeader className="border-b border-black/50 bg-white/60 backdrop-blur-xl">
                                        <CardTitle className="text-base font-semibold text-slate-900">Prescription</CardTitle>
                                        <CardDescription className="text-[12px] text-slate-600">
                                        ...
                                        {rxLoading ? (
                                            ...
                                            <>
                                                <Field label="Prescription notes (optional)">
                                                    <Textarea value={rx.notes} onChange={(e) => setRx((r) => ({ ...r, notes: e.target.value }))} className={UI.textarea} />
                                                </Field>
                                                ...
                                                <div className="flex items-center justify-between gap-2">
                                                    ...
                                                    <Button type="button" variant="outline" className="h-10 rounded-2xl border-black/50 bg-white/85 font-semibold" onClick={addRxRow}>
                                                        + Add medicine
                                                    </Button>
                                                </div>
                                                            ...
                                                            <div key={idx} className="rounded-3xl border border-black/50 bg-white/70 p-3">
                                                                <div className="grid gap-3 md:grid-cols-12 md:items-end">
                                                                    <div className="md:col-span-4">
                                                                        ...
                                                                        <Input value={it.drug_name || ''} onChange={(e) => onRxItem(idx, 'drug_name', e.target.value)} placeholder="e.g., Tab Paracetamol" className={UI.input} />
                                                                    </div>

                                                                    <div className="md:col-span-2">
                                                                        <div className={UI.label}>Strength</div>
                                                                        <Input value={it.strength || ''} onChange={(e) => onRxItem(idx, 'strength', e.target.value)} placeholder="500 mg" className={UI.input} />
                                                                    </div>

                                                                    <div className="md:col-span-3">
                                                                        <div className={UI.label}>Frequency</div>
                                                                        <Input value={it.frequency || ''} onChange={(e) => onRxItem(idx, 'frequency', e.target.value)} placeholder="1-0-1 / BD / TID" className={UI.input} />
                                                                    </div>

                                                                    <div className="md:col-span-1">
                                                                        <div className={UI.label}>Days</div>
                                                                        <Input type="number" value={it.duration_days ?? 0} onChange={(e) => onRxItem(idx, 'duration_days', e.target.value)} className={UI.input} />
                                                                    </div>

                                                                    <div className="md:col-span-1">
                                                                        <div className={UI.label}>Qty</div>
                                                                        <Input type="number" value={it.quantity ?? 0} onChange={(e) => onRxItem(idx, 'quantity', e.target.value)} className={UI.input} />
                                                                    </div>

                                                                    <div className="md:col-span-1 flex md:justify-end">
                                                                        <Button type="button" variant="outline" className="h-11 rounded-2xl border-black/50 bg-white/85 font-semibold" onClick={() => removeRxRow(idx)}>
                                                                            Remove
                                                                        </Button>
                                                                    </div>
                                                                </div>
                                                ...
                                                <div className={cx(UI.softBox, 'flex items-center justify-between gap-2')}>
                                                    ...
                                                    <Button type="button" className="h-11 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-semibold" onClick={saveRx} disabled={rxSaving}>
                                                        {rxSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />}
                                                        Save prescription
                                                    </Button>
                                                </div>
                                            </>
                                ...
                                <Card className={cx(UI.glass)}>
                                    ...
                                    <CardContent className="pt-4 space-y-4">
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Button type="button" variant="outline" className="h-10 rounded-2xl border-black/50 bg-white/85 font-semibold" onClick={copySummary}>
                                                    <Copy className="mr-2 h-4 w-4" />
                                                    Copy
                                                </Button>
                                                <Button type="button" variant="outline" className="h-10 rounded-2xl border-black/50 bg-white/85 font-semibold" onClick={printPdf} disabled={pdfing}>
